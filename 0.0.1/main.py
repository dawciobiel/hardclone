...
from PySide6.QtWidgets import (
    ...
    QGridLayout, QLabel, QComboBox, QPushButton, QLineEdit, QCheckBox,
    QSpinBox, QProgressBar, QTextEdit, QGroupBox, QFrame, QFileDialog,
    QMessageBox, QScrollArea, QSizePolicy, QSplitter, QTabWidget,
    QDialog, QDialogButtonBox, QInputDialog
)
from PySide6.QtCore import Qt, QTimer, Signal, QSize, QObject, QThread
from PySide6.QtGui import QFont, QPalette, QColor, QPixmap, QPainter, QBrush, QIcon
    ...
    def get_device_size(self):
        ...
        try:
            if self.sudo_password:
                cmd = f"echo '{self.sudo_password}' | sudo -S blockdev --getsize64 {self.source_device}"
                result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
            else:
                cmd = ["blockdev", "--getsize64", self.source_device]
...
class DDGUIManager(QMainWindow):
    ...
    def create_image(self):
        ...
        needs_sudo = self.check_sudo_needed(source_device)
        sudo_password = None
